    ...
    return (
      <div className="py-16">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <Paragraph>The user profile you're looking for doesn't exist or has been removed.</Paragraph>
        </div>
      </div>
    );
  }
        ...
        <div className="max-w-5xl mx-auto">
              ...
              <div className="flex flex-col md:flex-row gap-6 items-center md:items-start">
                ...
                <div className="flex-1 text-center md:text-left">
                  ...
                  <div className="flex flex-wrap gap-2 justify-center md:justify-start mb-4">
                    ...
                    <Badge variant="outline" className="bg-[#ccff00] bg-opacity-30 text-primary">
                      <BriefcaseIcon className="h-3 w-3 mr-1" />
                      {user.ideas.length} Projects
                    </Badge>
                  </div>
                  
                  {user.bio && <Paragraph className="mb-4">{user.bio}</Paragraph>}
          ...
          <Tabs defaultValue="projects">
            <TabsList className="mb-6">
              <TabsTrigger value="projects">Projects ({user.ideas.length})</TabsTrigger>
                  ...
                  <CardContent className="p-6">
                    <ul className="space-y-4">
                      {user.collaborations.map(collab => (
                        <li key={collab.id} className="border-b last:border-0 pb-4 last:pb-0">
                          <div className="flex justify-between items-start">
                            <div>
                              <p className="font-medium">Project ID: {collab.ideaId}</p>
                              <p className="text-sm text-gray-500">Status: {collab.status}</p>
                              <p className="text-sm mt-1">{collab.message}</p>
                            </div>
                            <Badge 
                              variant={collab.status === "accepted" ? "default" : "outline"}
                              className={collab.status === "accepted" ? "bg-green-500" : ""}
                            >
                              {collab.status}
                            </Badge>
                          </div>
